...
class MetricLearningDataset_pairwise(Dataset):
    ...
    def __getitem__(self, query_idx):
        ...
        index_id = int(self.query_id_2_index_id[query_id_orig])

        return query_name1, query_name2, index_id, query_id_orig
...
class MetricLearningDataset_pairwise2(Dataset):
    """
    Candidate Dataset for:
        query_tokens, candidate_tokens, label
    """
    def __init__(self, path, tokenizer, random_erase=0): #d_ratio, s_score_matrix, s_candidate_idxs):
        ...
        for line in lines:
            line = line.rstrip("\n")
            query_id, name1 = line.split("||")
            self.query_ids.append(query_id)
            self.query_names.append(name1)
            # self.query_names.append((name1, name1))#wei
        self.tokenizer = tokenizer
        self.query_id_2_index_id = {k: v for v, k in enumerate(list(set(self.query_ids)))}
        self.random_erase = random_erase
    ...
    def __getitem__(self, query_idx):
        ...
        query_name1 = self.query_names[query_idx]


        query_id_orig = self.query_ids[query_idx]
        index_id = int(self.query_id_2_index_id[query_id_orig])

        return query_name1, query_name1, index_id, query_id_orig
